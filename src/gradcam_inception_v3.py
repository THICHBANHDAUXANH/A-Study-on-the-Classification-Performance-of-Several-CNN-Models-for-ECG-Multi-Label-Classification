...
THRESHOLD = 0.5
WEIGHTS_PATH = MODEL_DIR/"best_inception_v3_ecg_model.pth"
OUTPUT_DIR = GRADCAM_DIR/"inception_v3"
...
def main() -> None:
    ...
    model = ECGInceptionV3(len(class_names)).to(device)
    model.load_state_dict(checkpoint["model_state_dict"])
    model.eval()
    cam = GradCAM(model)
    image_paths = sorted(path for path in IMAGE_DIR.glob("*.png") if path.stem.isdigit())

    for image_path in tqdm(image_paths, desc="Generating Inception-v3 Grad-CAM", unit="record"):
